...
## Analysis
First, the data was examined descriptively. 

### Principal Component Analysis

In PCA part, we focus on six numeric variables measuring the daily diet.
\par
- `DAILYFTJUICE`: Daily servings of juice
- `DAILYDGREENVEG`: Daily servings of dark green vagetables
- `DAILYFFRY`: Daily servings of Frech fries
- `DAILYPOTATO`: Daily servings of potatoes
- `DAILYFRT`: Daily servings of fruit
- `DAILYOTHVEG`: Daily servings of other vagetables
...
\par
The Table 3.2.1-1 shows the loadings of all six principal components (PCs). We can observe the proportion of variance explained (PVE) and cumulative PVE from Figure 3.2.1-1, Figure 3.2.1-2 and Table 3.2.1-2. The scree plots of variances and PVE indicate that the four leading PCs should be sufficient. We further explore the interpretations of loadings and PCs:
\begin{align*}
...
plot(summary(pc_daily)$importance[2, ],
     ...
     xlab="Number of PCs",
     pch = 16,
     main="Figure 3.2.1-1: Scree Plot of PVE")
```
...
plot(summary(pc_daily)$importance[3, ], pch=16,
     ylab="Cumulative PVE",
     xlab="Number of PCs",
     main="Figure 3.2.1-2: Scree Plot of Cumulative PVE")
```

```{r summary of PVE, results='asis',message=FALSE}
# summary of PVE
x <- summary(pc_daily)$importance 
knitr::kable(x, caption = "Table 3.2.1-2: Summary table of PVE")
```

The biplot (Figure 3.2.1-3) visualizes the PC scores together with the loadings of the original six variables. From the plot, we can see that the loadings for $PC_1$ have the same signs, while $PC_2$ captures the difference between the total intake of`DAILYDGREENVEG`, `DAILYFRT`, `DAILYOTHVEG` and `DAILYFTJUICE` versus`DAILYFTJUICE`, `DAILYFFRY`, and `DAILYPOTATO`.
...
ggbiplot(pc_daily, obs.scale = 1, var.scale = 1)+
  ...
  labs(title = "Figure 3.2.1-3: Biplot")+
  theme_bw()
...
We chose 12 numeric variables for clustering. We first performed PCA and used 8 PCs for further analysis based on the summary table (The Table 3.2.2-1) and the scree plot of PVE (The Figure 3.2.2-1). To determine the number of clusters, we calculated the total within-cluster sum of square (WSS) and decided to use 4 clusters (The Figure 3.2.2-2). The figure "True Diabetes Status VS Clusters" shows the comparison of the real diabetes status with the clustering by k-means (The Figure 3.2.2-3). **We cannot observe a clear pattern, indicating that we cannot use only the numeric variables to separate the diabetes status clearly.**
...
knitr::kable(y, caption = "Table 3.2.2-1: Summary table of PVE for clustering")
```
...
plot(summary(pca_all)$importance[2, ],
     ...
     xlab="Number of PCs",
     pch = 16,
     main="Figure 3.2.2-1: Scree Plot of PVE")
```
...
data.frame(x = pca_all$x[,1],
  ...
  scale_shape_manual(labels = c("Clulster 1", "Cluster 2","Cluster 3", "Cluster 4"),
                     values = c(4, 16, 8, 17)) +
  theme_bw() +
  labs(color = "Diabetes status", shape = "Cluster",title = "Figure 3.2.2-3: True Diabetes Status VS Clusters") +
  xlab("PC1") +
  ylab("PC2") 
```
